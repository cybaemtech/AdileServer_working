  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        // TEMPORARY FIX: Also treat 500 errors on auth endpoint as "not authenticated"
        // This handles IIS server configuration issues
        if (error?.status === 500) {
          console.warn('[useAuth] 500 error on auth endpoint - treating as not authenticated');
          return null;
        }
        throw error;
      }
