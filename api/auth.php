...
// Set CORS headers only if not already set by index.php
if (!headers_sent()) {
    setCorsHeaders();
}
...
if ($path === '' && isset($_SERVER['REQUEST_URI'])) {
    ...
    if (preg_match('~/api/auth(/.*)?$~i', $uriPath, $m)) { 
        $path = $m[1] ?? '/'; 
    }
}
// Default to /user for auth API calls without specific path
if ($path === '' || $path === '/') {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        $path = '/user';  // Default GET to /user endpoint
    } else {
        $path = '/'; 
    }
}
...
function forgotPassword($conn) {
    ...
    try {
        ...
        // Send email with new password using SMTP
        ...
        try {
            ...
            $smtp_success = $mailer->sendPasswordResetEmail($email, $user['full_name'] ?: $user['username'], $newPassword);
            ...
            error_log("SMTP Mailer failed: " . $e->getMessage());
        }
