...
export class DatabaseStorage {
  ...
  async createWorkItem(insertWorkItem: InsertWorkItem): Promise<WorkItem> {
    ...
    // Ensure screenshot_blob is a Buffer (binary) if present and is base64
    let screenshotBlob: Buffer | null = null;
    let screenshotPath: string | null = null;
    if ((insertWorkItem as any).screenshot_blob) {
      const blob = (insertWorkItem as any).screenshot_blob;
    ...
    if ((insertWorkItem as any).screenshot_path || (insertWorkItem as any).screenshotPath) {
      screenshotPath = (insertWorkItem as any).screenshot_path || (insertWorkItem as any).screenshotPath;
    }
    ...
    const result = await db.insert(workItems).values({
      ...
      createdByName,
      createdByEmail,
      actualHours: insertWorkItem.actualHours !== undefined && insertWorkItem.actualHours !== null && (insertWorkItem.actualHours as any) !== "" ? Number(insertWorkItem.actualHours) : null,
  ...
  async updateWorkItem(id: number, updates: Partial<WorkItem>): Promise<WorkItem | undefined> {
    ...
    if (updates.screenshotPath !== undefined) processedUpdates.screenshot_path = updates.screenshotPath;
    if ((updates as any).screenshot_blob !== undefined) processedUpdates.screenshot_blob = (updates as any).screenshot_blob;
    if ((updates as any).screenshot_path !== undefined) processedUpdates.screenshot_path = (updates as any).screenshot_path;
    
    // Remove the camelCase versions before sending to DB
